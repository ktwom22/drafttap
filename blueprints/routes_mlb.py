...
def get_df_raw():
    try:
        # Cache-bust every 2 mins to prevent Railway network hanging
        sync_url = f"{SALARY_CSV}&t={int(time.time() / 120)}"
        ...
        for col, target in [('salary', 'Salary'), ('proj', 'Proj_Base')]:
            ...
            if found:
                df[target] = pd.to_numeric(df[found].astype(str).replace(r'[^0-9.]', '', regex=True), errors='coerce').fillna(0.0)
            else:
                df[target] = 0.0
...
def run_optimizer(df_input, num_lineups=1, locks=[], stack_team=None, min_stack=4,
    ...
    if excluded_games:
        df = df[~df.apply(lambda r: " vs ".join(sorted([TEAM_MAP.get(str(r['Team']), str(r['Team'])),
                                                        TEAM_MAP.get(str(r['Opponent']), str(r['Opponent']))])) in excluded_games, axis=1)]

    all_results, used_player_indices = [], []
    valid_teams = df[~df['POS'].str.contains('P')].groupby('Team').filter(lambda x: len(x) >= min_stack)['Team'].unique().tolist()

    if stack_team and stack_team != "None":
        target_teams = [stack_team] if stack_team in valid_teams else []
    else:
        target_teams = df[~df['POS'].str.contains('P')].groupby('Team')['Proj_Base'].nlargest(6).groupby('Team').mean().sort_values(ascending=False).index.tolist()
    ...
    for i in range(num_lineups):
        ...
        if not stack_team or stack_team == "None": random.shuffle(target_teams)

        for current_team in target_teams[:12]: # Slightly throttled for cloud resources
            try:
                ...
                def get_modified_proj(row):
                    ...
                    return p + random.uniform(-0.1, 0.1)
                ...
                for p in indices:
                    ...
                    for s in SLOTS:
                        valid = (s.startswith('P') and 'P' in pos) or (s == 'C' and 'C' in pos) or \
                                ('1B' in s and '1B' in pos) or ('2B' in s and '2B' in pos) or \
                                ('3B' in s and '3B' in pos) or ('SS' in s and 'SS' in pos) or \
                ...
                # Railway Safety: Strict 2s limit per solve attempt
                prob.solve(pulp.PULP_CBC_CMD(msg=0, timeLimit=2.0))
                ...
                if pulp.LpStatus[prob.status] == 'Optimal':
                    ...
                    if score > highest_score:
                        ...
                        best_lineup = {'players': l_players, 'total_salary': int(t_sal), 'total_projection': round(t_proj, 2), 'indices': p_indices}
            except Exception as e:
                print(f"Solver iteration error: {e}")
                continue
...
def index():
    ...
    # Fast Lookups (Call these once)
    weather = get_mlb_weather_data()
    espn = get_espn_game_info()
    h_fg, p_fg = get_weighted_stats()
    dynamic_matchup = get_prime_matchup()

    # SPEED FIX: Dictionary Mapping (Replaces Fuzzy Matching inside the loop)
    p_map = p_fg.set_index('full_name').to_dict('index') if not p_fg.empty else {}
    h_map = h_fg.set_index('full_name').to_dict('index') if not h_fg.empty else {}
    choices_p = list(p_map.keys())
    choices_h = list(h_map.keys())

    pool_ui, game_map = [], {}

    # Iterate using records for O(1) access
    for row in df_raw.to_dict('records'):
        ...
        # Fast Fuzzy Logic
        try:
            target_choices = choices_p if is_p else choices_h
            target_map = p_map if is_p else h_map
            match = process.extractOne(name, target_choices, scorer=fuzz.token_set_ratio)
            if match and match[1] >= 85:
                adv = target_map.get(match[0], {})
                mlb_id = str(adv.get('mlb_id', '0'))
        except: pass

        t1, t2 = TEAM_MAP.get(row['Team'], row['Team']), TEAM_MAP.get(row['Opponent'], row['Opponent'])
        ...
        pool_ui.append({
            ...
            'Match_Display': f"{time_val} @ {w_data.get('venue', 'Ballpark')} | {w_data.get('temp', '--')}° {w_data.get('condition', '')}",
        ...
        if g_id not in game_map:
            game_map[g_id] = {'id': g_id, 'display': g_id, 'time': time_val, 'sort': e_data.get('raw_time', datetime.max.replace(tzinfo=pytz.utc))}
    ...
    if request.method == 'POST':
        try:
            ...
            exposure_limits = {p: max(1, int((min(float(request.form.get(f'exposure_{p}', 100)), global_cap) / 100) * num_lineups)) for p in df_raw['Player'].unique()}

            results = run_optimizer(df_raw, num_lineups=num_lineups, locks=request.form.getlist('player_locks'),
                                    stack_team=request.form.get('stack_team'), min_stack=int(request.form.get('min_stack', 4)),
                                    diversity=int(request.form.get('diversity', 4)), excluded_games=request.form.getlist('excluded_games'),
                                    exposure_limits=exposure_limits)
            status = f"OPTIMIZED {len(results)} LINEUPS" if results else "FAILED: CHECK CONSTRAINTS"
        except Exception as e:
            print(f"POST ERROR: {e}")
            status = "OPTIMIZER ERROR"
